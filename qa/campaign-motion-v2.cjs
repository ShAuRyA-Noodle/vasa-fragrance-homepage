...
(async()=>{
 ...
 for(const [width,height,reduced] of [[1440,1000,false],[768,900,false],[390,844,false],[320,760,true]]){
  ...
  assert.deepEqual(await page.locator('.story-slide img').evaluateAll(images=>images.map(image=>image.complete&&image.naturalWidth>0)),[true,true,true,true],'all story images must be decoded before the first transition');
  ...
  if(width>760){
   ...
   const progressBefore=Number(await page.locator('.story-tab[aria-pressed="true"]').evaluate(element=>getComputedStyle(element).getPropertyValue('--fill')||0));
   await page.waitForTimeout(700);
   const progressAfter=Number(await page.locator('.story-tab[aria-pressed="true"]').evaluate(element=>getComputedStyle(element).getPropertyValue('--fill')||0));
   assert.ok(progressAfter>progressBefore+.04,'story progress remains paused after using a carousel control');
   ...
   const giftGeometry=await page.evaluate(()=>{const art=document.querySelector('.gift-art').getBoundingClientRect(),copy=document.querySelector('.gift-copy').getBoundingClientRect();return {artRight:Math.round(art.right),copyLeft:Math.round(copy.left)}});
   assert.ok(giftGeometry.artRight<=giftGeometry.copyLeft+1,'gifting image overlaps its copy');
