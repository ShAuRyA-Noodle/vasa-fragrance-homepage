...
import { createVelvetBottle } from './model/createVelvetBottle.js';
...
export function createVelvetAtmosphere(host, { stage } = {}) {
  ...
  scene.add(new THREE.HemisphereLight(0xffe9df, 0x462434, 2.2));
  const key = new THREE.DirectionalLight(0xffe2ca, 3.7);
  key.position.set(-4, 5, 7);
  scene.add(key);
  const rim = new THREE.DirectionalLight(0xd68bb6, 3.1);
  rim.position.set(3.7, 2.1, -4.4);
  scene.add(rim);
  const glint = new THREE.PointLight(0xffbc83, 5.2, 8, 2);
  glint.position.set(-3, -1.2, 1.4);
  scene.add(glint);

  const bottle = createVelvetBottle();
  ...
  const sweep = new THREE.SpotLight(0xffd8d5, 0, 9, Math.PI / 8, .75, 1.6);
  ...
  const fog = new THREE.Mesh(new THREE.PlaneGeometry(15, 10), new THREE.ShaderMaterial({
    ...
    fragmentShader: `varying vec2 uv;uniform float time;uniform float progress;uniform float strength;
      ...
      void main(){vec2 p=uv*.43+vec2(time*.035+progress*.3,-time*.012);float mist=n(p*3.)*.62+n(p*6.)*.38;
        float a=smoothstep(.48,.75,mist)*strength*.18;gl_FragColor=vec4(mix(vec3(.59,.26,.41),vec3(.94,.66,.48),progress*.5),a);}`,
  }));
  fog.position.z = -4.3;
  scene.add(fog);
  ...
  for (let i = 0; i < 5; i += 1) {
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `varying vec2 uv;uniform float alpha;uniform float warm;void main(){
        ...
        vec3 tint=mix(vec3(.95,.57,.75),vec3(1.,.7,.45),warm);
  ...
  // Curved air streams are tube geometry, not a 2D overlay. They sweep across
  ...
  const ingredientSpecs = [
    { url: '/media/campaign/scent-elements/almonds-open-shell.webp', at: .49, x: -2.9, y: -.8, z: .7, size: 1.85 },
    { url: '/media/campaign/scent-elements/tuberose-flowers.webp', at: .69, x: 2.9, y: .45, z: -.2, size: 2.2 },
    { url: '/media/campaign/scent-elements/tonka-beans-split-pod.webp', at: .86, x: -2.7, y: -1.1, z: .55, size: 2 },
  ];
  ...
  const fleckMaterial = new THREE.PointsMaterial({
    color: 0xffd7d4, size: .035, sizeAttenuation: true,
    transparent: true, opacity: 0, depthWrite: false,
    blending: THREE.AdditiveBlending,
  });
