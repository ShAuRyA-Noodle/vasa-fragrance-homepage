...
export function createStormAtmosphere(host, { stage } = {}) {
  ...
  scene.add(new THREE.HemisphereLight(0xe1f3f4, 0x344b55, 2.2));
  const key = new THREE.DirectionalLight(0xfff2dc, 3.7);
  key.position.set(-4, 5, 7);
  scene.add(key);
  const rim = new THREE.DirectionalLight(0x9fdaeb, 3.1);
  ...
  const sweep = new THREE.SpotLight(0xd9f6ff, 0, 9, Math.PI / 8, .75, 1.6);
  ...
  const ingredientSpecs = [
    { url: '/media/campaign/scent-elements/bergamot-peel.webp', at: .49, x: -2.9, y: -.8, z: .7, size: 1.85 },
    { url: '/media/campaign/scent-elements/lavender-sprigs.webp', at: .69, x: 2.9, y: .45, z: -.2, size: 2.2 },
    { url: '/media/campaign/scent-elements/cedar-shavings-branch.webp', at: .86, x: -2.7, y: -1.1, z: .55, size: 2 },
  ];
  ...
  const fleckMaterial = new THREE.PointsMaterial({
    color: 0xdaf2ee, size: .035, sizeAttenuation: true,
    transparent: true, opacity: 0, depthWrite: false,
    blending: THREE.AdditiveBlending,
  });
