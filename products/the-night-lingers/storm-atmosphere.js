...
import { createNightBottle } from './model/createNightBottle.js';
...
export function createNightAtmosphere(host, { stage } = {}) {
  ...
  scene.add(new THREE.HemisphereLight(0xffe9cc, 0x4f2815, 2.2));
  const key = new THREE.DirectionalLight(0xffd49b, 3.7);
  key.position.set(-4, 5, 7);
  scene.add(key);
  const rim = new THREE.DirectionalLight(0xffa852, 3.1);
  rim.position.set(3.7, 2.1, -4.4);
  scene.add(rim);
  const glint = new THREE.PointLight(0xffb95c, 5.2, 8, 2);
  glint.position.set(-3, -1.2, 1.4);
  scene.add(glint);

  const bottle = createNightBottle();
  ...
  const sweep = new THREE.SpotLight(0xffdeae, 0, 9, Math.PI / 8, .75, 1.6);
  ...
  const fog = new THREE.Mesh(new THREE.PlaneGeometry(15, 10), new THREE.ShaderMaterial({
    ...
    fragmentShader: `varying vec2 uv;uniform float time;uniform float progress;uniform float strength;
      ...
      void main(){vec2 p=uv*.43+vec2(time*.035+progress*.3,-time*.012);float mist=n(p*3.)*.62+n(p*6.)*.38;
        float a=smoothstep(.48,.75,mist)*strength*.18;gl_FragColor=vec4(mix(vec3(.71,.37,.15),vec3(1.,.72,.34),progress*.5),a);}`,
  }));
  fog.position.z = -4.3;
  scene.add(fog);
  ...
  for (let i = 0; i < 5; i += 1) {
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `varying vec2 uv;uniform float alpha;uniform float warm;void main(){
        ...
        vec3 tint=mix(vec3(1.,.63,.29),vec3(1.,.84,.51),warm);
  ...
  const ingredientSpecs = [
    { url: '/media/campaign/scent-elements/oud-wood-chip.webp', at: .49, x: -2.9, y: -.8, z: .7, size: 1.85 },
    { url: '/media/campaign/scent-elements/brown-sugar-crystals.webp', at: .69, x: 2.9, y: .45, z: -.2, size: 2.2 },
    { url: '/media/campaign/scent-elements/incense-resin-smoke.webp', at: .86, x: -2.7, y: -1.1, z: .55, size: 2 },
  ];
  ...
  const fleckMaterial = new THREE.PointsMaterial({
    color: 0xffd9a3, size: .035, sizeAttenuation: true,
    transparent: true, opacity: 0, depthWrite: false,
    blending: THREE.AdditiveBlending,
  });
