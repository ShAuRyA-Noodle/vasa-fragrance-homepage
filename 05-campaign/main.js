...
if(giftingGallery)giftingGallery.innerHTML=giftIdeas.map((gift,i)=>`<a class="mood-gallery-panel gift-gallery-panel${i===0?' is-active':''}" href="/gifting/" aria-label="Explore ${gift.title.toLowerCase()}"><img src="${gift.image}" alt="" loading="lazy" width="1600" height="1000"><span class="mood-gallery-shade"></span><span class="mood-gallery-copy"><small>${gift.label}</small><strong>${gift.title}</strong><em>${gift.line}</em><span class="gift-gallery-link">EXPLORE GIFTS <b aria-hidden="true">↗</b></span></span></a>`).join('');
...
function renderBag(){const items=bag.items;content.innerHTML=items.length?`${items.map(i=>{const p=productById(i.id);return `<article class="bag-item">${imageMarkup(p)}<div><h3>${p.name}</h3><small>50 ml · Price announced at launch</small><div class="quantity"><button data-quantity="${p.id}" data-value="${i.quantity-1}" aria-label="Decrease ${p.name} quantity">−</button><span aria-label="Quantity">${i.quantity}</span><button data-quantity="${p.id}" data-value="${i.quantity+1}" ${i.quantity===9?'disabled':''} aria-label="Increase ${p.name} quantity">+</button><button class="remove" data-remove="${p.id}">Remove</button></div></div></article>`}).join('')}<p class="bag-status">Your selection is saved on this device. Ordering will open once prices and availability are confirmed. No payment is taken.</p><a class="button dark panel-action" href="/bag/">VIEW YOUR BAG ${icon}</a>`:`<p class="panel-note">Your bag is waiting for a fragrance that feels like you.</p><a class="button dark" href="/collection/">EXPLORE THE COLLECTION ${icon}</a>`;}
...
function searchResults(query=''){const q=query.toLocaleLowerCase().trim();const results=catalog.filter(p=>[p.name,p.family,p.description,...p.notes.flat()].join(' ').toLocaleLowerCase().includes(q));document.querySelector('#search-results').innerHTML=results.length?results.map(p=>`<a class="search-result" href="/products/${p.id}/">${imageMarkup(p)}<span><strong>${p.name}</strong><small>${p.family}</small></span></a>`).join(''):`<p class="panel-note">No fragrances found for “${escapeText(query)}”. Try a name or note, such as rose or cedar.</p>`;document.querySelector('#search-count').textContent=`${results.length} fragrance${results.length===1?'':'s'} found`;requestAnimationFrame(()=>animatePanelContent('search'));}
const simplePanels={
...
function open(name){if(name==='bag')return bagPanel();if(name==='search'){showPanel('search','Find your fragrance','<label class="search-label" for="fragrance-search">Search by name or fragrance note</label><input class="panel-search" id="fragrance-search" type="search" placeholder="Try rose, cedar, or a fragrance name" autocomplete="off"><p class="sr-only" id="search-count" aria-live="polite"></p><div class="search-results" id="search-results"></div>');searchResults();content.querySelector('input').addEventListener('input',e=>searchResults(e.target.value));return;}
 if(name==='menu'){showPanel('menu','Explore VASA','<div class="menu-links"><a href="/collection/">Collection</a><a href="/gifting/">Gifting</a><a href="/our-story/">Our Story</a><a href="/contact/">Contact</a><a href="/feel-your-fragrance/">Feel your fragrance</a><a href="/build-your-fragrance/">Build your own</a><button data-panel="profile">Your profile</button></div>');requestAnimationFrame(()=>animatePanelContent('menu'));return;}
...
document.addEventListener('click',e=>{const el=e.target.closest('button,a');if(!el)return;if(el.matches('a[href="/feel-your-fragrance/"],a[href="/build-your-fragrance/"]')){e.preventDefault();const feel=el.getAttribute('href').includes('feel');showPanel('soon',feel?'Feel your fragrance':'Build your own',`<p class="panel-kicker">COMING SOON</p><p class="panel-note soon-note">${feel?'An immersive way to discover the scent that is yours.':'Compose a fragrance around who you are.'}</p><p class="panel-note">We are putting the final touches on it. Until then, meet the collection.</p><a class="button dark" href="/collection/">EXPLORE THE COLLECTION ${icon}</a>`);return;}if(el.dataset.panel)open(el.dataset.panel);else if(el.dataset.product)productPanel(el.dataset.product);else if(el.dataset.add){bag.add(el.dataset.add);toast(`${productById(el.dataset.add).name} added to your bag`);if(panel.open)bagPanel();}else if(el.dataset.quantity){const id=el.dataset.quantity,increment=el.textContent.trim()==='+';bag.set(id,Number(el.dataset.value));const controls=[...content.querySelectorAll(`[data-quantity="${id}"]`)];const target=controls.find(b=>(b.textContent.trim()==='+')===increment&&!b.disabled)||controls.find(b=>!b.disabled)||content.querySelector('[data-quantity], [data-continue]');target?.focus();}else if(el.dataset.remove){bag.remove(el.dataset.remove);content.querySelector('[data-remove], [data-continue]')?.focus();}else if(el.hasAttribute('data-continue')){closePanel();requestAnimationFrame(()=>document.querySelector('#collection').scrollIntoView({behavior:reduced.matches?'instant':'smooth'}));}else if(el.hasAttribute('data-dismiss')){closePanel();}});
